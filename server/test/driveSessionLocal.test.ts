...
async function run(): Promise<void> {
  ...
  const finished = await finishActiveStory(session.id, 'skipped', session.journeyState.getActiveMoment()?.momentId);
  ...
  try {
    ...
    await finishActiveStory(city.id, 'ended', city.journeyState.getActiveMoment()?.momentId);
